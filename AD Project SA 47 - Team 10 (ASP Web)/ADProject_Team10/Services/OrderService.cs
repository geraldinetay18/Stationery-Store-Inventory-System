...
namespace ADProject_Team10.Services
{
    ...
    public class OrderService : IOrderService
    {
        ...
        public int FindLastPoNumber()
        {
            SSAEntities context = new SSAEntities();
            return (context.Orders.ToList().Count == 0) ? 1000000000 : context.Orders.Max(x => x.PoNumber);
        }
        public int FindLastDoNumber()
        {
            SSAEntities context = new SSAEntities();
            return (context.Orders.ToList().Count == 0) ? 1100000000 : context.Orders.Max(x => x.DoNumber);
        }
    }
}