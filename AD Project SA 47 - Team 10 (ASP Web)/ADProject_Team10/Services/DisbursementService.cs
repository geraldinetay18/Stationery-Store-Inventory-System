...
namespace ADProject_Team10.Services
{
    public class DisbursementService : IDisbursementService
    {
        ...
        {
            return se.Disbursements.Where(d => d.Status == "Acknowledged" && d.DeptId == id).FirstOrDefault();
        }
        ...
        public List<DisbursementDetail> GetDisbursementStationeryByDeptId_Date(string depId, DateTime fromdate, DateTime toDate)
        {
            ...
            try
            {
                ...
                List<DisbursementDetail> list = ssa.DisbursementDetails.Where(x => x.Disbursement.DeptId.Equals(depId)
                 ...
                 && x.DisbursementId == x.Disbursement.DisbursementId && x.Disbursement.Status == "Acknowledged").ToList();
                return list;
            }
            catch (Exception ex)
            {
